package com.west.auth.oauth2.resolver;
 ...
 * @date 2021/3/4 10:16
 */
public class GiteeOAuth2AuthorizationRequestResolver implements OAuth2AuthorizationRequestResolver {
    private static final String WEIXIN_DEFAULT_SCOPE = "user_info";
    private static final String REGISTRATION_ID_URI_VARIABLE_NAME = "registrationId";

    private final OAuth2AuthorizationRequestResolver defaultAuthorizationRequestResolver;
    private final AntPathRequestMatcher authorizationRequestMatcher;

    public GiteeOAuth2AuthorizationRequestResolver(ClientRegistrationRepository clientRegistrationRepository) {
    ...
    private OAuth2AuthorizationRequest resolve(OAuth2AuthorizationRequest authorizationRequest, String registrationId) {
        ...
        // 如不是WeiXin则使用默认实现
        if (!OAuth2GiteeContants.GITEE_REGISTRATION_ID.equals(registrationId)) {
            return authorizationRequest;
        }

        // 微信Authorization Request URL
        String authorizationRequestUri = String.format(OAuth2GiteeContants.GITEE_AUTHORIZATION_REQUEST_URL_FORMAT,
                authorizationRequest.getAuthorizationUri(), authorizationRequest.getClientId(),
                encodeURL(authorizationRequest.getRedirectUri()), authorizationRequest.getResponseType().getValue());
